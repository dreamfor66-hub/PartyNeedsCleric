...
public class BuffManager
{
    ...
    public void RemoveById(string buffId, int stackCount = 1)
    {
        if (string.IsNullOrEmpty(buffId) || stackCount <= 0)
            return;

        int remain = stackCount;

        for (int i = list.Count - 1; i >= 0 && remain > 0; i--)
        {
            var inst = list[i];
            if (inst == null || inst.Data == null || inst.Data.buffId != buffId)
                continue;

            int remove = Mathf.Min(remain, inst.Stacks);
            inst.RemoveStacks(remove);
            remain -= remove;
        }
    }

    public void RemoveByTag(BuffTag tag, int stackCount = 1)
    {
        if (tag == BuffTag.None || stackCount <= 0)
            return;
        ...
        for (int i = list.Count - 1; i >= 0 && remain > 0; i--)
        {
            ...
            if (inst.Data.tags == null || !inst.Data.tags.Contains(tag)) continue;
            ...
            switch (a.effect)
            {
                ...
                case BuffEffect.RemoveBuffById:
                    if (!string.IsNullOrEmpty(a.removeBuffId) && targetObj is CharacterBehaviour rmChar)
                        rmChar.Buffs.RemoveById(a.removeBuffId, GetFinalEffectCount(a, ctx));
                    break;
                case BuffEffect.RemoveBuffByTag:
                    if (a.effectBuffTag != BuffTag.None && targetObj is CharacterBehaviour tagChar)
                        tagChar.Buffs.RemoveByTag(a.effectBuffTag, GetFinalEffectCount(a, ctx));
                    break;
                case BuffEffect.RemoveBuff:
                    if (ctx.ThisBuff != null)
                        ctx.ThisBuff.RemoveStacks(GetFinalEffectCount(a, ctx));
                    break;
